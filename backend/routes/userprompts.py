...
def create_user_prompt(prompt: schemas.PromptCreate, db: Session = Depends(get_db)):
...
def read_user_prompts(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    prompts = db.query(models.UserPrompt).offset(skip).limit(limit).all()
    return prompts

@router.get("/{prompt_id}", response_model=schemas.Prompt)
def read_user_prompt(prompt_id: int, db: Session = Depends(get_db)):
...
def delete_user_prompt(prompt_id: int, db: Session = Depends(get_db)):
    prompt = db.query(models.UserPrompt).filter(models.UserPrompt.id == prompt_id).first()
    if prompt is None:
        raise HTTPException(status_code=404, detail="Prompt not found")
    db.delete(prompt)
...
def update_user_prompt(prompt_id: int, prompt: schemas.PromptCreate, db: Session = Depends(get_db)):
